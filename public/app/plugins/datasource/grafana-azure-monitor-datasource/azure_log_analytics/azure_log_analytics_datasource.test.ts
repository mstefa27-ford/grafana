...
import AzureLogAnalyticsDatasource from './azure_log_analytics_datasource'; // will use the version in __mocks__
...
describe('AzureLogAnalyticsDatasource', () => {
  ...
  describe('When the cloudName contains china', () => {
    beforeEach(() => {
      ctx.instanceSettings = {
        jsonData: {
          logAnalyticsSubscriptionId: 'xxx',
          cloudName: 'chinaloganalyticsazure',
        },
    ...
    it('should set baseUrl to chinaloganalyticsazure', () => {
      const azureLogAnalyticsDatasource: AzureLogAnalyticsDatasource = new AzureLogAnalyticsDatasource(
        ctx.instanceSettings,
        ctx
      );
      expect(azureLogAnalyticsDatasource.baseUrl).toEqual('/chinaloganalyticsazure');
    });
  });

  describe('When the cloudName does not contains china', () => {
    beforeEach(() => {
      ctx.instanceSettings = {
        jsonData: {
          logAnalyticsSubscriptionId: 'xxx',
    ...
    it('should set baseUrl to loganalyticsazure', () => {
      const azureLogAnalyticsDatasource: AzureLogAnalyticsDatasource = new AzureLogAnalyticsDatasource(
        ctx.instanceSettings,
        ctx
      );
      expect(azureLogAnalyticsDatasource.baseUrl).toEqual('/loganalyticsazure');
    });
